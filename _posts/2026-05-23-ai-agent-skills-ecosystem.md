---
layout: post
title: "AI agent skillsはプロンプト集じゃなくて、作業を持ち運ぶための層になり始めている"
date: 2026-05-23 11:30:00 +0900
categories: [ai, agents]
tags: [agent-skills, claude-code, codex, cursor, agents-md, mcp]
summary: "Claude Code skills、AGENTS.md、Cursor rules、MCP配布、X上の実例を横断して、AI agent skillsがプロンプト集から作業手順・安全ガード・配布・検証のレイヤーへ移りつつある流れを見る。"
---

AI agent skills を少し追ったら、思っていたより早く景色が変わっていた。

最初は「Claude Codeに入れる便利プロンプト集」くらいに見ていた。でも、今日ざっとGitHub、公式ドキュメント、Zenn、Xを横断してみると、もう少し違う。

今起きているのは、**人間とエージェントが繰り返している作業を、持ち運べる単位に切り出す動き** だと思う。

プロンプトを書く話ではある。けれど、それだけではない。
...
つまり、skills は「賢い指示文」ではなく、**作業の型を保存・配布・検証するための薄い運用レイヤー** になり始めている。
...
これは「プロンプトを書く」より、「作業ログから手順書とテストを生成する」に近い。
...
skillsは、プロンプトをうまく書く話から、**反復作業をどう抽出し、どう安全に実行し、どう検証し、どう複数agentへ配るか** という話に移っている。
